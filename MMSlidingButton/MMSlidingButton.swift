...
@IBDesignable class MMSlidingButton: UIView {
    ...
    @IBInspectable var imageName: UIImage = .init() {
        didSet {
            setStyle()
        }
    }
    ...
    func setUpButton() {
        ...
        self.dragPoint = UIView(frame: CGRect(
            x: dragPointWidth - self.frame.size.width,
            y: 0,
        ...
        if self.imageName != UIImage() {
            self.imageView = UIImageView(frame: CGRect(
                x: self.frame.size.width - dragPointWidth,
                y: 0,
    ...
    func unlock() {
        ...
        UIView.transition(with: self, duration: 0.2, options: .curveEaseOut, animations: {
            self.dragPoint.frame = CGRect(
                x: self.frame.size.width - self.dragPoint.frame.size.width,
                y: 0,
    ...
    func reset() {
        ...
        UIView.transition(with: self, duration: 0.2, options: .curveEaseOut, animations: {
            self.dragPoint.frame = CGRect(
                x: self.dragPointWidth - self.frame.size.width,
                y: 0,
                width: self.dragPoint.frame.size.width,
                height: self.dragPoint.frame.size.height)
        }, completion: { status in
